...
const CongressionalColumns: Column[] = [
    {
        accessorKey: "candidate",
        header: "Candidatos",
        cell: ({ row, index, isExpanded= undefined }) => {
            return (
                <TableCell key={row.id*2} className={`min-w-[300px] align-top bg-white ${index == 0 && 'sticky left-0'}`}>
                    <TableCellViewer item={row}/>
                </TableCell>
            )
        },
    ...
    },
    {
        ...
        cell: ({ row, isExpanded= undefined, setExpanded, navigate}) => {
            return (
                <TableCell key={row.id * 13} className="min-w-[300px]">    
                   { row.postulaDepartamento}
                </TableCell>
        ...
        <Drawer direction={isMobile ? "bottom" : "right"}>
            <DrawerTrigger asChild className="group select-none cursor-pointer">
                <div className="flex items-center gap-3">
                    {
                        item.cargo[0] == PositionOrderEnum.PRESIDENTIAL || PositionOrderEnum.FIRST_VICE_PRESIDENT || PositionOrderEnum.SECOND_VICE_PRESIDENT || item.cargo[0] == PositionOrderEnum.PARLIAMENTARY
                            ?
                            <>
